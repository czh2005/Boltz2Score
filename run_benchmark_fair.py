...
def run_benchmark():
    ...
    for i, fasta in enumerate(fastas):
        ...
        # Copy processed data from previous run to avoid MSA rate limits and measure pure inference time
        src_processed = output_dir("benchmark_out_full_20_real") / f"boltz_results_{fasta_path.stem}/processed"
        dst_dir = output_dir("benchmark_out_full_fair") / f"boltz_results_{fasta_path.stem}"
        dst_processed = dst_dir / "processed"
        ...
        try:
            server.predict(
                ...
                out_dir=output_dir("benchmark_out_full_fair"),
                score_only=False
            )
    ...
    for i, fasta in enumerate(fastas):
        ...
        # Copy processed data to avoid MSA rate limits
        src_processed = output_dir("benchmark_out_full_fair") / f"boltz_results_{fasta_path.stem}/processed"
        dst_dir = output_dir("benchmark_out_score_fair") / f"boltz_results_{fasta_path.stem}"
        dst_processed = dst_dir / "processed"
        ...
        input_struct = prediction_cif("benchmark_out_full_fair", fasta_path.stem)
        ...
        try:
            server.predict(
                data_path=str(fasta_path),
                out_dir=output_dir("benchmark_out_score_fair"),
                score_only=True,
