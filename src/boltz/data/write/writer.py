        ...
        for record, coord, pad_mask in zip(records, coords, pad_masks):
            ...
            for model_idx in range(coord.shape[0]):
                ...
                if "confidence_score" in prediction:
                    ...
                    if "iptm" in prediction and prediction["iptm"][model_idx].item() > 0:
                        confidence_summary_dict["iptm"] = prediction["iptm"][model_idx].item()
                    elif "ptm" in prediction:
                        confidence_summary_dict["ptm"] = prediction["ptm"][model_idx].item()
                    with path.open("w") as f:
                        f.write(
                            json.dumps(
                                confidence_summary_dict,
