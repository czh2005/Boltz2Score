...
def compute_ptms(logits, x_preds, feats, multiplicity, score_only=False):
    # It needs to take as input the mask of the frames as they are not used to compute the PTM
    if score_only:
        # For score_only, we don't compute collinear mask, just use token_pad_mask
        B = x_preds.shape[0]
        mask_collinear_pred = torch.ones(B // multiplicity, multiplicity, feats["token_pad_mask"].shape[1], device=x_preds.device)
        mask_collinear_pred = mask_collinear_pred * feats["token_pad_mask"][:, None, :]
    else:
